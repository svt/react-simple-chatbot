...
const getData = (params, callback) => {
  const { cacheName, cache, firstStep, steps, historySteps, botAvatar, userAvatar } = params;

  const sanitizedHistorySteps = historySteps.map(step => ({
    ...step,
    key: Random(24),
    avatar: step.user ? userAvatar : botAvatar,
    rendered: true,
    delay: 0
  }));

  const currentStep = firstStep;
  const renderedSteps = [...sanitizedHistorySteps, steps[currentStep.id]];
